...
export class EmailService {
    ...
    scheduledEmailTs(mailbox: MailBox): number {
        ...
        const timeTakenToDeliverAllEmails =
            Math.ceil(emailsToDeliver / mailsPer10Minutes) * 10 * 60 * 1000;
    ...
    async updateUrlOpened(id: number, eid: number): Promise<string> {
        const urlShortner = await this.usRepository.findOne({
            where: { id: id },
        });
        if (!urlShortner) {
    ...
    async fetchDeliveredEmails(
        mailbox: MailBox,
        durationInHrs: number = 10,
    ): Promise<Email[]> {
        const now = new Date();
